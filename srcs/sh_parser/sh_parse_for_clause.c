...
/*   Updated: 2016/08/28 01:45:43 by juloo            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool			sh_parse_for_clause(t_sh_parser *p, t_sh_cmd *dst)
{
	...
	if (!sh_except_token(p, SH_PARSE_T(TEXT)))
		return (false);
	if (!is_identifier(p->l.t.token_str))
		return (sh_parse_error(p, SH_E_INVALID_ID));
	dst->for_clause = MALLOC(sizeof(t_sh_for) + p->l.t.token_str.length);
	dst->for_clause->var = SUB_DST(ENDOF(dst->for_clause), p->l.t.token_str);
	sh_ignore_newlines(p);
	if (sh_except_token(p, SH_PARSE_T(TEXT))
		&& (SUB_EQU(p->l.t.token_str, SUBC("in"))
